...
On controller restart, the original launch configuration first passes the existing training and batch validation gates. The controller then reads the latest complete Torch checkpoint's `config_path` and restores its saved microbatch, learning rate, precision, worker count, and prefetch factor. The files must belong to the same model and checkpoint identity within the project; the small configuration JSON must match its committed digest. The saved microbatch is bounded by the validated calibration and the learning rate by the original configuration. Ordinary resume clears stale `recovery_overrides`, so the trainer's unchanged recipe guard applies and the saved event schedule continues without another recovery event or initial save. New failure-triggered adjustments retain their explicit recovery authorization.
